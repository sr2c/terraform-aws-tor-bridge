output "ansible_inventory" {
  value       = "${module.this.id} ansible_user=${var.ssh_user} ansible_host=${module.instance.public_ip}"
  sensitive   = true
  description = "An Ansible inventory line that allows login to the bridge with the correct username."
}

output "bridgeline" {
  value = (module.instance.public_ip == null) ? null : replace(
    replace(
      replace(module.bridgeline.stdout,
      "<IP ADDRESS>", module.instance.public_ip),
